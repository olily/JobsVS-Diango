...
class WizardFormPlugin(BaseAdminPlugin):
    ...
    def prepare_form(self, __):
        ...
        if self.request.method == 'GET':
            ...
            if wizard_goto_step and int(
                    wizard_goto_step) < len(self.get_form_list()):
                obj = self.get_form_list().keys()
                if six.PY3:
                    obj = [s for s in obj]
    ...
    def get_form_layout(self, __):
        ...
        if isinstance(attrs, dict) and 'layout' in attrs:
    ...
    def get_all_cleaned_data(self):
        ...
        for form_key, attrs in self.get_form_list().items():
            ...
            if form_obj.is_valid():
                if isinstance(attrs, dict) and 'convert' in attrs:
