...
def insertDB(table_name, index_id,sql_value):
    ...
    try:
        # 执行sql语句
        cursor.execute(
            'insert into city_info values( %d, %d,  "%s", "%s", "%s", "%s", "%s", "%s","%s", "%s", "%s", "%s", "%s", "%s","%s", "%s","%s")'
            % (index_id,sql_value[0],str(sql_value[1]),str(sql_value[2]),str(sql_value[3]),str(sql_value[4]),str(sql_value[5]),
               str(sql_value[6]), str(sql_value[7]), str(sql_value[8]),str(sql_value[9]),str(sql_value[10]),str(sql_value[11]),
               str(sql_value[12]), str(sql_value[13]), str(sql_value[14]), str(sql_value[15])))
        # 提交到数据库执行
        db.commit()
    except BaseException:
        # 如果发生错误则回滚
...
jobs_urls.to_csv('data/mphone_jobs_urls.csv')
