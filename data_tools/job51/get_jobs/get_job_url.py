...
def turn_page_thread(submission):
    city_id = submission[0]
    city_code = submission[1]
    global num
    ...
    while (True):
        ...
        if len(jobs_list) ==1:
            print(datetime.datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S'),city_id, city_code,num,"完成")
            break

        # print(jobs_list)

        company_code=salary_type=put_time=None
        ...
        for items in jobs_list:
            ...
            row = [job_name,job_code,company_code,salary_type,salary_low,salary_high,city_id,put_time,job_url]
            # print(row)
