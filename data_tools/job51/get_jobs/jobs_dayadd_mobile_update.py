...
def insertDB(sql_value):
    ...
    # 提交到数据库执行,每1000条提交一次
    # save_num+=1
    sql = 'update jobs_get set job_fun = "%s",status_update=1 where code="%s"' % (str(sql_value[0]),str(sql_value[1]))
    # print(save_num)
    fp.write(sql + ';\n')
    fp.flush()
    # exit()
...
f = open("job_update.sql", "w+", encoding="utf-8")
f.write('-- '+datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '\n')
f.close()

fp = open("job_update.sql", "a+", encoding="utf-8")
