...
def spider_page(submission):
    ...
    for item in industry_items:
        # 保存行业分类
        category_name = item.span.text
        sql = 'insert into industries(name,industry_id,url) values("%s","%s", "%s")' % (category_name, j,None)
        cursor.execute(sql)
        j+=1
    db.commit()
...
def insertDB(sql_value):
    ...
    # 提交到数据库执行,每1000条提交一次
    sql = 'insert into industries(name,industry_id,url,category_id) values("%s","%s", "%s",%d)' % (str(sql_value[0]), str(sql_value[1]),str(sql_value[2]), sql_value[3])
    # SQL 插入语句
    try:
        mylock2.acquire()
        save_num += 1
...
fp = open("data/industry_errsql.txt", "a+", encoding="utf-8")
...
submission = ["https://www.51job.com/sitemap/trade_Navigate.php",user_agent_list[0]]
