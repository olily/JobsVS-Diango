...
def cursor_query(user_agent_list):
    ...
    sscursor.execute('select code,url from jobs_get_copy1')
    while True:
...
def insertDB(sql_value):
    ...
    # 提交到数据库执行,每1000条提交一次
    sql = 'update jobs_get_copy1 set education="%s",work_year="%s",jobfare = "%s",job_fun = "%s" where code="%s"' % (str(sql_value[0]),str(sql_value[1]),str(sql_value[2]),str(sql_value[3]),str(sql_value[4]))
    print(sql)
    # SQL 插入语句
    try:
        mylock2.acquire()
